...
# ---------------------------------------------------------------------------
# Feature flags — flip based on installed versions
# ---------------------------------------------------------------------------

# TRL >= 0.12 renamed `max_seq_length` -> `max_length` in SFTConfig
SFT_USES_MAX_LENGTH = TRL >= (0, 12, 0)

# TRL >= 0.12 config classes moved to `processing_class` instead of `tokenizer`
TRAINER_USES_PROCESSING_CLASS = TRL >= (0, 12, 0)

# TRL >= 0.13 added `loss_type` variants and `mask_truncated_completions`
HAS_MASK_TRUNCATED = TRL >= (0, 13, 0)

# transformers >= 4.46 renamed `torch_dtype` kwarg on from_pretrained
